"""Targeted re-chunk of the fraud born-digital sha that were force-visioned
(_fraud_borndigital_done_sha.txt). Reuses the exact build_email_chunks_v2
functions so chunk docs match the main run, deleting old chunks and re-inserting
with fresh contextual summaries + embeddings from the new frontier OCR text.
"""
...
SHA_FILE = "_fraud_borndigital_done_sha.txt"
...
def main() -> int:
    ...
    try:
        ...
        shas = sorted({ln.strip() for ln in Path(SHA_FILE).read_text(
            encoding="utf-8").splitlines() if ln.strip()})
        logger.info(f"re-chunking {len(shas)} fraud born-digital sha")
